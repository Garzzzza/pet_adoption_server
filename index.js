...
const { getAllPetsModel, getPetByIdModel, postPetByModel, updatePetModel, getSearchedPetsModel, } = require("./models/petModel")
const { getAllUsersModel, postUserByModel, getUserByIdModel, updateUserModel, } = require("./models/signModel")

const { savePetModel, getSavedPetsModel, deleteSavedPetModel, adoptPetModel, getAdoptedPetsModel, deleteAdoptedPetModel,
    fosterPetModel, getFosteredPetsModel, deleteFosteredPetModel,
} = require("./models/petUserModel")
const dbConnection = require('./knex/knex')
const { passwordMatch, isNewUser, hashPwd, isExistingUser, comparePass, auth, upload, } = require("./middlewares/middlewares")
...
app.get("/pets", async (request, response) => {
    try {
        ...
        if (request.query.adoptionStatus) queryObj.adoptionStatus = request.query.adoptionStatus;
        if (request.query.height > 0) queryObj.height = parseInt(request.query.height);
        if (request.query.weight > 0) queryObj.weight = parseInt(request.query.weight);
...
app.delete("/savedpets/:userId/:petId", auth, async (request, response) => {
    try {
        const isDeleted = await deleteSavedPetModel(request.params.userId, request.params.petId)
...
app.post("/fosteredpets/:userId/:petId", auth, async (req, res) => {
    try {
        const petToFoster = await fosterPetModel(req.params.userId, req.params.petId)
...
app.delete("/fosteredpets/:userId/:petId", auth, async (request, response) => {
    try {
        const isDeleted = await deleteFosteredPetModel(request.params.userId, request.params.petId)
        response.send({ isDeleted })
...
app.delete("/adoptedpets/:userId/:petId", auth, async (request, response) => {
    try {
        const isDeleted = await deleteAdoptedPetModel(request.params.userId, request.params.petId)
        response.send({ isDeleted })
...
app.post("/users/signup", /*validateBody(signupSchema)*/ passwordMatch, isNewUser, hashPwd, async (request, response) => {
    const newUser = {
...
app.post("/users/login", isExistingUser, comparePass, async (request, response) => {
    try {
        const token = jwt.sign({ id: request.body.user.userId, isAdmin: request.body.user.isAdmin },
