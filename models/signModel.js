...
async function getUserByEmailModel(email) {
    try {
        const user = await dbConnection('users_table').where({ signUpEmail: email }).first()
        return user
    } catch (err) {
        console.log(err)
    }
...
async function getUserByIdModel(userId) {
    try {
        const user = await dbConnection('users_table').where({ userId: userId }).first()
        return user
    } catch (err) {
        console.log(err)
    }
...
module.exports = {
    getAllUsersModel, postUserByModel, getUserByEmailModel, getUserByIdModel, updateUserModel,
};
